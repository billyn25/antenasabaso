...
function serviceCards(town=''){
  return services.map((s,i)=>`<article class="service-card${i===0?' featured':''}"><span class="num">${String(i+1).padStart(2,'0')}</span><h3>${esc(s.name)}${town?' en '+esc(town):''}</h3><p>${esc(s.text)}</p></article>`).join('');
}
