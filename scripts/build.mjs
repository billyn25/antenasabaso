...
function localTerritory(province,town){
  return `<section class="local-territory"><div class="wrap local-territory-inner"><div><span class="eyebrow">Contexto territorial</span><h2>${esc(town)} · ${esc(province.name)}</h2><p>${esc(town)} forma parte del Territorio Histórico de ${esc(province.name)}. Desde esta página puedes consultar los servicios disponibles en la localidad y volver al directorio completo de ${province.towns.length} municipios de ${esc(province.name)}.</p></div><a class="province-back" href="/${province.slug}/">Ver municipios de ${esc(province.name)} →</a></div></section>`;
}
